...
- 🧍 **Körper-Kachel** (`type: body`): stilisierte, freundlich abstrahierte Silhouette (handgetunte Formen schlank/normal/kräftig × weiblich/männlich, gemorpht nach Gewicht vs. Ziel) — ein Arm ruht (trägt die Blutdruck-Manschette), der andere zeigt den Bizeps (für den Muskelmasse-Anker); Energie-Glow nach Gesamtscore, Fieber-Glow an Kopf/Brust, angedeutete Augenringe bei Schlafmangel, frei platzierbare Wert-Anker (`anchors`). Wählbarer **Figur-Stil** per `figure_style`: `svg` (gezeichnet, Default), `flat` (flache Silhouette), `glass` (Liquid Glass), `mannequin` (Schaufensterpuppe), `pixar` (weiches 3D) — die passenden Bilder (weiblich/männlich × unter-/normal-/übergewichtig) sind mitgeliefert und werden nach Geschlecht + Gewichtszustand automatisch gewählt; Glow/Fieber/Anker funktionieren weiter. Anker frei platzieren mit `x`/`y` (Prozent), z. B. `{ entity: ..., x: 33, y: 16 }`. Alternativ eigene Renderings per `images: { slim, regular, full }`.
