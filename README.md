...
- 🧍 **Körper-Kachel** (`type: body`): stilisierte, freundlich abstrahierte Silhouette (handgetunte Formen schlank/normal/kräftig × weiblich/männlich, gemorpht nach Gewicht vs. Ziel) — ein Arm ruht (trägt die Blutdruck-Manschette), der andere zeigt den Bizeps (für den Muskelmasse-Anker); Energie-Glow nach Gesamtscore, Fieber-Glow an Kopf/Brust, angedeutete Augenringe bei Schlafmangel, frei platzierbare Wert-Anker (`anchors`). Alternativ eigene Renderings per `images: { slim, regular, full }` — die Karte wählt nach Gewichtszustand, Glow/Fieber/Anker funktionieren weiter. Bilder auf schwarzem Hintergrund (KI-Renderings): `image_remove_black: true` macht Schwarz per Luminanz→Alpha transparent (Light & Dark tauglich). Anker frei platzieren mit `x`/`y` (Prozent), z. B. `{ entity: ..., x: 22, y: 16 }`
