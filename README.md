...
- 🧍 **Körper-Kachel** (`type: body`): gerenderte Figur (weiblich/männlich × unter-/normal-/übergewichtig, gewählt nach Geschlecht + Gewicht vs. Ziel) mit Energie-Glow nach Gesamtscore, Fieber-Glow an Kopf/Brust, angedeuteten Augenringen bei Schlafmangel und frei platzierbaren Wert-Ankern (`anchors`). **Figur-Stil** per `figure_style`: `mannequin` (Default), `glass` (Liquid Glass, Default bei Glass-Karte), `pixar` (weiches 3D), `flat` (flache Silhouette) — alle Bilder sind mitgeliefert. Anker frei platzieren mit `x`/`y` (Prozent), z. B. `{ entity: ..., x: 33, y: 16 }`. Alternativ eigene Renderings per `images: { slim, regular, full }`.
